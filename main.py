...
CSV_URLS = [
    "https://lotto-datenbank.de/download/euro_jackpot.csv",
    "https://lotto-datenbank.de/download/euro_jackpot_absteigend.csv",
]
...
def load_from_csv_fallback() -> pd.DataFrame | None:
    for url in CSV_URLS:
        try:
            print(f"Pokušavam CSV fallback: {url}")
            r = requests.get(url, headers=HEADERS, timeout=30)
            if r.status_code != 200:
                print(f"CSV nije dostupan, status={r.status_code}")
                continue
            ...
            print(f"CSV učitan, stupci: {list(df.columns)}")

            cols = {c.lower().strip(): c for c in df.columns}

            date_col = None
            for candidate in ["datum", "date", "draw date"]:
                if candidate in cols:
                    date_col = cols[candidate]
                    break

            if date_col is None:
                print("CSV nema datum stupac")
                continue
            ...
            for c in df.columns:
                cl = c.lower().strip()
                if cl in ["n1", "n2", "n3", "n4", "n5", "br 1", "br 2", "br 3", "br 4", "br 5"]:
                    main_candidates.append(c)
                if cl in ["e1", "e2", "d 1", "d 2", "*1", "*2", "ez1", "ez2"]:
                    euro_candidates.append(c)

            if len(main_candidates) < 5 or len(euro_candidates) < 2:
                print("CSV nema prepoznatih 5+2 stupaca")
                continue

            main_candidates = main_candidates[:5]
            euro_candidates = euro_candidates[:2]

            out = pd.DataFrame()
            out["Datum"] = df[date_col]
            out["date"] = pd.to_datetime(df[date_col], errors="coerce", dayfirst=True)

            for i, c in enumerate(main_candidates, start=1):
                out[f"main_{i}"] = pd.to_numeric(df[c], errors="coerce")

            for i, c in enumerate(euro_candidates, start=1):
                out[f"euro_{i}"] = pd.to_numeric(df[c], errors="coerce")

            if "kolo" in cols:
                out["Kolo"] = pd.to_numeric(df[cols["kolo"]], errors="coerce")

            out = out.dropna(subset=["date"]).sort_values("date").reset_index(drop=True)

            if not out.empty:
                print(f"CSV fallback uspješan, učitano {len(out)} kola")
                return out

        except Exception as e:
            print(f"Greška na CSV fallbacku: {e}")
            continue
...
def load_all_history() -> pd.DataFrame:
    csv_df = load_from_csv_fallback()
    if csv_df is not None and not csv_df.empty:
        return csv_df

    print("CSV fallback nije uspio, pokušavam scraping po godinama...")
    current_year = dt.datetime.now().year
    frames = []

    for year in range(2012, current_year + 1):
        print(f"Dohvaćam godinu {year}...")
        df = fetch_year(year)
        if df is not None and not df.empty:
            frames.append(df)

    if not frames:
        raise RuntimeError("Nema dohvaćenih podataka sa izvora (CSV i scraping fallback nisu uspjeli).")
    ...
    for c in ["main_1", "main_2", "main_3", "main_4", "main_5", "euro_1", "euro_2", "Kolo"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")

    df = df.sort_values("date").reset_index(drop=True)
    print(f"Scraping fallback uspješan, učitano {len(df)} kola")
    return df
...
def merge_scores(freq_df, gap_df, rec_df):
...
def build_messages(df5: pd.DataFrame):
    ...
    main_scores = merge_scores(main_freq, main_gap, main_rec)
    euro_scores = merge_scores(euro_freq, euro_gap, euro_rec)
    ...
    if not generated:
        raise RuntimeError("Model nije uspio generirati kombinacije.")
    ...
    odd_even_str = ", ".join([f"{k[0]}N/{k[1]}P ({int(v)}x)" for k, v in odd_even.head(3).items()])
    low_high_str = ", ".join([f"{k[0]} low/{k[1]} high ({int(v)}x)" for k, v in low_high.head(3).items()])
...
def send_telegram(text: str):
    ...
    if not bot_token or not chat_id:
        print("Telegram secreti nisu postavljeni.")
        print(text)
        return
...
def main():
    ...
    print("Pokrećem dohvat povijesti...")
    df = load_all_history()
    print(f"Ukupno učitano kola: {len(df)}")
    print(df.tail(3).to_string())

    df5 = subset_5y(df)
    print(f"Kola u zadnjih 5 godina: {len(df5)}")

    prediction, evaluation = build_messages(df5)

    if mode == "EVALUATION":
        send_telegram(evaluation)
    else:
        send_telegram(prediction)
